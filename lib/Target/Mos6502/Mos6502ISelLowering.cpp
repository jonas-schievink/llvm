...
#define DEBUG_TYPE "Mos6502-lower"
...
SDValue Mos6502TargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  switch (Op.getOpcode()) {
  case ISD::BR_CC:
...
SDValue Mos6502TargetLowering::LowerFormalArguments(
  ...
  for (auto &VA : ArgLocs) {
    if (VA.isRegLoc()) {
      ...
      /*case MVT::i8:
        unsigned VReg = RegInfo.createVirtualRegister(&Mos6502::GPRRegClass);
...
SDValue Mos6502TargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
                                     SmallVectorImpl<SDValue> &InVals) const {
  ...
  // Mos6502 target does not support tail call optimization.
  IsTailCall = false;
  ...
  CCInfo.AnalyzeCallOperands(Outs, CC_Mos6502);
  ...
  Chain = DAG.getNode(Mos6502ISD::CALL, CLI.DL, NodeTys, Ops);
  InFlag = Chain.getValue(1);
...
Mos6502TargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
  ...
  unsigned Opc = Mos6502ISD::RET_FLAG;
...
SDValue Mos6502TargetLowering::LowerCallResult(
  ...
  CCInfo.AnalyzeCallResult(Ins, RetCC_Mos6502);
...
SDValue Mos6502TargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(Mos6502ISD::BR_CC, DL, Op.getValueType(), Chain, LHS, RHS,
                     DAG.getConstant(CC, DL, MVT::i64), Dest);
}

SDValue Mos6502TargetLowering::LowerSELECT_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(Mos6502ISD::SELECT_CC, DL, VTs, Ops);
}

const char *Mos6502TargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch ((Mos6502ISD::NodeType)Opcode) {
  case Mos6502ISD::FIRST_NUMBER:
    break;
  case Mos6502ISD::RET_FLAG:
    return "Mos6502ISD::RET_FLAG";
  case Mos6502ISD::CALL:
    return "Mos6502ISD::CALL";
  case Mos6502ISD::SELECT_CC:
    return "Mos6502ISD::SELECT_CC";
  case Mos6502ISD::BR_CC:
    return "Mos6502ISD::BR_CC";
  case Mos6502ISD::Wrapper:
    return "Mos6502ISD::Wrapper";
  }
  return nullptr;
}

void Mos6502TargetLowering::ReplaceNodeResults(SDNode *N,
...
SDValue Mos6502TargetLowering::LowerGlobalAddress(SDValue Op,
                                                  SelectionDAG &DAG) const {
  // Build a `tglobaladdr` and put it behind a custom Wrapper node
  SDLoc DL(Op);
...
/*
MachineBasicBlock *
Mos6502TargetLowering::EmitInstrWithCustomInserter(MachineInstr *MI,
                                               MachineBasicBlock *BB) const {
  const TargetInstrInfo &TII = *BB->getParent()->getSubtarget().getInstrInfo();
  DebugLoc DL = MI->getDebugLoc();

  assert(MI->getOpcode() == Mos6502::Select && "Unexpected instr type to insert");
  ...
  switch (CC) {
  case ISD::SETGT:
    BuildMI(BB, DL, TII.get(Mos6502::JSGT_rr))
        .addReg(LHS)
        .addReg(RHS)
        .addMBB(Copy1MBB);
    break;
  case ISD::SETUGT:
    BuildMI(BB, DL, TII.get(Mos6502::JUGT_rr))
        .addReg(LHS)
        .addReg(RHS)
        .addMBB(Copy1MBB);
    break;
  case ISD::SETGE:
    BuildMI(BB, DL, TII.get(Mos6502::JSGE_rr))
        .addReg(LHS)
        .addReg(RHS)
        .addMBB(Copy1MBB);
    break;
  case ISD::SETUGE:
    BuildMI(BB, DL, TII.get(Mos6502::JUGE_rr))
        .addReg(LHS)
        .addReg(RHS)
        .addMBB(Copy1MBB);
    break;
  case ISD::SETEQ:
    BuildMI(BB, DL, TII.get(Mos6502::JEQ_rr))
        .addReg(LHS)
        .addReg(RHS)
        .addMBB(Copy1MBB);
    break;
  case ISD::SETNE:
    BuildMI(BB, DL, TII.get(Mos6502::JNE_rr))
        .addReg(LHS)
        .addReg(RHS)
        .addMBB(Copy1MBB);
    break;
  ...
  // ...
  BB = Copy1MBB;
  BuildMI(*BB, BB->begin(), DL, TII.get(Mos6502::PHI), MI->getOperand(0).getReg())
